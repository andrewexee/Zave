        ...
        <div className="md:hidden mt-4 flex flex-col gap-2 border-t border-zinc-800 pt-4">
          ...
          <Link
            ...
            className={`flex items-center justify-between px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
              isActive('/cart') ? 'bg-yellow-400 text-black' : 'text-zinc-400 hover:text-white hover:bg-zinc-800'
            }`}
          >
            ...
            {cartCount > 0 && (
              <span className={`w-5 h-5 text-xs font-bold rounded-full flex items-center justify-center leading-none ${
                isActive('/cart') ? 'bg-black text-yellow-400' : 'bg-yellow-400 text-black'
              }`}>
                {cartCount > 9 ? '9+' : cartCount}
              </span>
            )}
