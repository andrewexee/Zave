  ...
  return (
    <nav className="bg-zinc-900 border-b border-zinc-800 px-4 md:px-6 py-4">
      <div className="flex items-center justify-between">
        ...
        {/* Lado Izquierdo: Logo (Ocupa espacio para centrar el medio) */}
        ...
        {/* Móvil: Logo simple a la izquierda */}
        <span className="md:hidden text-2xl font-black tracking-tighter text-white">
          Z<span className="text-yellow-400">a</span>ve
        </span>

        {/* Centro: Menú de navegación (Centrado matemático) */}
        ...
        {/* Lado Derecho: Usuario (Alineado al final, ocupa espacio simétrico) */}
        <div className="hidden md:flex md:flex-1 md:basis-0 items-center justify-end gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col text-right">
              <span className="text-white text-sm font-medium leading-none">{profile?.name}</span>
              <span className="text-xs font-mono mt-0.5 text-zinc-500">{profile?.role}</span>
            </div>
          </div>
          <button
            onClick={signOut}
        ...
        <button
          ...
          className="md:hidden flex flex-col gap-1.5 p-2 rounded-lg hover:bg-zinc-800"
        >
          <span className={`block w-5 h-0.5 bg-white transition-transform ${menuOpen ? 'rotate-45 translate-y-2' : ''}`} />
          <span className={`block w-5 h-0.5 bg-white ${menuOpen ? 'opacity-0' : ''}`} />
          <span className={`block w-5 h-0.5 bg-white transition-transform ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
        </button>
